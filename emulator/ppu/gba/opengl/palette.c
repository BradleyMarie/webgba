...
void OpenGlBgPaletteReload(OpenGlBgPalette* context, const GbaPpuMemory* memory,
  ...
  if (dirty_bits->palette.bg_palette) {
    ...
    glBindTexture(GL_TEXTURE_2D, context->bg_large_palette);
    ...
    glBindTexture(GL_TEXTURE_2D, context->bg_small_palette);
    glTexSubImage2D(GL_TEXTURE_2D, /*level=*/0, /*xoffset=*/0, /*yoffset=*/0,
...
void OpenGlBgPaletteReloadContext(OpenGlBgPalette* context) {
  glGenTextures(1u, &context->bg_large_palette);
  glBindTexture(GL_TEXTURE_2D, context->bg_large_palette);
  ...
  glGenTextures(1u, &context->bg_small_palette);
  glBindTexture(GL_TEXTURE_2D, context->bg_small_palette);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
...
void OpenGlBgPaletteDestroy(OpenGlBgPalette* context) {
  glDeleteTextures(1u, &context->bg_small_palette);
  glDeleteTextures(1u, &context->bg_large_palette);
