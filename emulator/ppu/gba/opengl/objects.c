...
bool OpenGlObjectsStage(OpenGlObjects* context, const GbaPpuMemory* memory,
  ...
  if (dirty_bits->oam.transformations) {
    ...
    dirty_bits->oam.transformations = false;
    context->transformations_dirty = true;
  }

  if (dirty_bits->oam.attributes || dirty_bits->io.obj_mosaic) {
    ...
    dirty_bits->oam.attributes = false;
    dirty_bits->io.obj_mosaic = false;
    context->attributes_dirty = true;
  }

  return context->transformations_dirty || context->attributes_dirty;
}
...
void OpenGlObjectsReload(OpenGlObjects* context) {
  if (context->transformations_dirty) {
    context->transformations_texture_index += 1u;
    if (context->transformations_texture_index == GBA_SCREEN_HEIGHT) {
      context->transformations_texture_index = 0u;
    }
    ...
    context->transformations_dirty = false;
  }

  if (context->attributes_dirty) {
    context->attributes_texture_index += 1u;
    if (context->attributes_texture_index == GBA_SCREEN_HEIGHT) {
    ...
    context->attributes_dirty = false;
  }
