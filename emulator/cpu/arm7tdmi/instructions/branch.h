...
static inline void ArmB(ArmUserRegisters *registers,
                        int_fast32_t offset) {
  assert(-8388608 <= offset && offset <= 8388607);
  const static uint_fast8_t offset_shift[2] = {2, 1};
  offset = (int32_t)((uint32_t)offset << offset_shift[registers->cpsr.thumb]);
