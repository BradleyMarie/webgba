...
class ExecuteTest : public testing::TestWithParam<uint16_t> {
 ...
 protected:
  ...
  bool RunInstruction(std::string instruction_hex) {
    if (instruction_hex[0] == '0' && instruction_hex[1] == 'x') {
      instruction_hex.erase(0, 2);
    }

    assert(instruction_hex.size() % 2 == 0);
    for (size_t i = 0; i < instruction_hex.size(); i += 2) {
      std::string hex_byte;
      hex_byte += instruction_hex[i];
      hex_byte += instruction_hex[i + 1];
...
std::vector<char> ExecuteTest::memory_space_(1024, 0);

TEST_F(ExecuteTest, ArmLDMDA) {
  registers_.current.user.gprs.r0 = 1;
